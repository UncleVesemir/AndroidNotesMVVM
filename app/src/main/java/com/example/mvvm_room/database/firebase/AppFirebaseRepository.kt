...
class AppFirebaseRepository: DatabaseRepository {
    ...
    override suspend fun delete(note: AppNote, onSuccess: () -> Unit) {
        FirebaseDatabase.getInstance().reference.child(NOTES).child(CURRENT_ID).child(note.idFirebase).removeValue()
                .addOnSuccessListener { onSuccess() }
                .addOnFailureListener { showToast(it.message.toString()) }
    }
    ...
    private fun initRefs() {
        ...
        REF_DATABASE = FirebaseDatabase.getInstance().reference.child(NOTES).child(CURRENT_ID)
    }
